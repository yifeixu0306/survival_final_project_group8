...
ggplot(lung_df_pie_1, aes(x="", y=labels, fill=Sex)) + 
...
ggplot(lung_df_pie_2, aes(x="", y=labels, fill=ph.ecog)) + 
  geom_bar(width = 1, stat = "identity", color="white", alpha = 0.7) + 
  geom_label(aes(label = labels),
             position = position_stack(vjust = 0.5),
             show.legend = FALSE,
...
df_karno %>%
  ggplot(aes(x=Karnofsky, fill = Rate, color = Rate)) + 
...
## 4 Hypothesis testing (identify the prognostic factors)
...
fit_ecog = survfit(Surv(log(time), status==1) ~ ph.ecog, data = lung_ecog)
...
```{r, fig.width=6}
# fit
lung_sex_ecog = lung_df %>%
 drop_na(sex, ph.ecog) %>%
  mutate(ph.ecog = as.factor(ph.ecog)) %>%
  mutate(Sex = case_when(sex == 1 ~ "Male",
                         sex == 2 ~ "Female")) 

fit_sex_ecog = survfit(Surv(time, status==1) ~ Sex + ph.ecog, data = lung_sex_ecog)
...
# log-rank test
diff_sex_ecog = survdiff(Surv(time, status==1) ~ Sex + ph.ecog, data = lung_sex_ecog) 
diff_sex_ecog
```
We find that female with ecog = 1 has a higher probability than male with ecog = 0.

## 4.2.2 age + ph.ecog

```{r, fig.height=6}
# fit
lung_age_ecog = lung_df %>%
  drop_na(age, ph.ecog) %>%
  mutate(Age = case_when(age > 70 ~ "> 70",
                         age <= 70 ~ "<= 70")) 

fit_age_ecog = survfit(Surv(time, status==1) ~ Age + ph.ecog, data = lung_age_ecog)
...
# log-rank test
diff_age_ecog = survdiff(Surv(time, status==1) ~ Age + ph.ecog, data = lung_age_ecog) 
diff_age_ecog
```
...
fit_ph_pat = survfit(Surv(time, status==1) ~ ph.karno + pat.karno, data=lung_phkarno)
summary(fit_ph_pat)$table

# visualization
ggsurvplot(fit_phkarno,
           pval = TRUE, conf.int = TRUE,
           surv.median.line = "hv", 
           ggtheme = theme_bw(), 
           legend = "bottom")
...
lung_mod = lung_df %>%
  drop_na() %>%
  mutate(age_grp = case_when(age > 70 ~ "> 70",
                         age <= 70 ~ "<= 70")) %>%
  mutate(age_grp = as.factor(age_grp)) %>%
  mutate(ph.ecogn = as.numeric(ph.ecog))
...
cox_mod_2 = coxph(Surv(time, status==1) ~ sex + ph.ecog + ph.karno + wt.loss, data = lung_mod)
summary(cox_mod_2)
```
...
```{r}
cox_mod_3 = coxph(Surv(time, status==1) ~ sex + ph.ecog + ph.karno + wt.loss + ph.karno*ph.ecog, 
                  data = lung_mod)
summary(cox_mod_3)
```
...
anova(cox_mod_full, cox_mod_1, cox_mod_2, cox_mod_3)
```
...
test_ecog <- survfit(Surv(time, status==1), status==1 ~ ph.ecog, data = data_ecog)
...
```{r}
cox_mod_4 = coxph(Surv(time, status == 1) ~ sex + ph.ecog + ph.karno + wt.loss +
                  sex*log(time) + ph.ecog*log(time) + ph.karno*log(time)  + wt.loss*log(time) 
                  - log(time), 
                  lung_mod) 
...
```{r}
cox_mod_5 = coxph(Surv(time, status==1) ~ ph.ecog + ph.karno + wt.loss:strata(sex), lung_mod) 
summary(cox_mod_5)
...
### 6.5.2 Time-varying covariates

```{r}
# explore ph.karno
fit2 = coxph(Surv(time, status==1) ~ ph.karno, data = lung_mod)
zph = cox.zph(fit2)

plot(zph[1],lwd=2) 
abline(0,0, col=1,lty=3,lwd=2)
abline(h= fit2$coef[1], col=3, lwd=2, lty=2) 
legend("bottomright",
...
fit_split = coxph(Surv(tstart, time, status) ~ sex + ph.ecog + ph.karno:strata(tgroup) + wt.loss, data=lung_split)
...
```{r}
# exponential
fit_exp = survreg(Surv(time, status==1) ~ sex + age + ph.ecog + ph.karno + pat.karno + meal.cal + wt.loss,
                  data = lung_mod, dist = "exp")
summary(fit_exp)

# weibull
fit_weibull = survreg(Surv(time, status==1) ~ sex + age + ph.ecog + ph.karno + pat.karno + meal.cal + wt.loss,
                      data = lung_mod, dist = "weibull")
summary(fit_weibull)

# log-logistics
fit_llogis = survreg(Surv(time, status==1) ~ sex + age + ph.ecog + ph.karno + pat.karno + meal.cal + wt.loss,
                     data = lung_mod, dist = "loglogistic")
summary(fit_llogis)

# log-normal
fit_lnorm = survreg(Surv(time, status==1) ~ sex + age + ph.ecog + ph.karno + pat.karno + meal.cal + wt.loss,
...
```{r}
# full weibull model
web = psm(Surv(time, status==1) ~ sex + age + ph.ecog + ph.karno + pat.karno + meal.cal + wt.loss, data = lung)

# backward selection
fastbw(web, rule="aic")

# compare with cox model
web_1 = phreg(Surv(time, status==1) ~ sex + ph.ecog,
                      data = lung_mod, dist = "weibull")
cox_1 = coxreg(Surv(time, status==1) ~ sex + ph.ecog, data = lung_mod)

check.dist(web_1, cox_1)

# final model
web_final = survreg(Surv(time, status==1) ~ sex + ph.ecog,
                      data = lung_mod, dist = "weibull")
# Scale parameter
web_final$scale
...
## AFT interpretation
## Geometric mean of survival time: 480
## female extends survival time by 1.439 times then male
## ecog = 1 shortens survival time by 0.811 times than ecog = 0
## ecog = 2 shortens survival time by 0.513 times than ecog = 0
## ecog = 3 shortens survival time by 0.246 times than ecog = 0
```



```{r}
openxlsx::write.xlsx(lung, file = "/Users/effy/Desktop") 
```
